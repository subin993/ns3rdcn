...
QueueBase::SetMaxSize (QueueSize size)
{
  ...
  m_maxSize = size;
  // m_maxSize = QueueSize (PACKETS, 10);
  m_maxSize = QueueSize (PACKETS, 3000);
  // m_maxSize = QueueSize (PACKETS, 100000);
  // m_maxSize = QueueSize (PACKETS, 1000000000);
  // NS_LOG_UNCOND(m_maxSize);
  NS_ABORT_MSG_IF (size < GetCurrentSize (),
                   "The new maximum queue size cannot be less than the current size");
}

void
QueueBase::SetMaxSize2 (int maxsize)
{
  // NS_LOG_FUNCTION (this << size);
  ...
  // m_maxSize = size;
  // m_maxSize = QueueSize (PACKETS, 10);
  m_maxSize = QueueSize (PACKETS, maxsize);
  // m_maxSize = QueueSize (PACKETS, 100000);
  // m_maxSize = QueueSize (PACKETS, 1000000000);
  // NS_LOG_UNCOND(m_maxSize);
  // NS_ABORT_MSG_IF (size < GetCurrentSize (),
