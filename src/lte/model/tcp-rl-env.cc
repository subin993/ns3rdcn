...
TcpGymEnv::ExecuteActions(Ptr<OpenGymDataContainer> action)
{
  ...
  // Hands-on
  if ((PktLoss == true)&(box->GetValue(1)>50000))
  {
...
TcpEventGymEnv::GetObservation()
{
  ...
  // New
  Ptr<OpenGymDictContainer> container = CreateObject<OpenGymDictContainer>();
  container -> Add("TcpData",box);
  NS_LOG_INFO ("MyGetObservation: " << container);
  return container;
...
TcpEventGymEnv::GetSsThresh (Ptr<const TcpSocketState> tcb, uint32_t bytesInFlight)
{
  ...
  // Hands-on
  PktLoss = true;
  std::cout<<"Packet was lost"<<std::endl;
